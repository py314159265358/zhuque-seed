...
| `seed.enc` | Fernet 密文种子 **v3.2**（**6884 字节，SHA-256 开头 `cf40f276`**），解出 3781 字符 = 13 节凭据（含 QQ 邮箱 + dedao） | 走活路 A 的新沙盒 |
...
**校验必须对上**：6884 字节、SHA-256 开头 `cf40f276`。对不上 = 载荷被换过，停止并报告。
解密密钥不在本仓，按 `SEED-PUBLIC.md` 第三节的路径找。

> ⚠️ **体积不再是判据**：v3.1 与 v3.2 **同为 6884 字节**（Fernet 长度只取决于明文，换密钥不变）。
> 只比体积会误判成「没换过」——**唯一可靠判据是 SHA-256**（v3.1 `78f6cab4` → v3.2 `cf40f276`）。
...
- 最近一次 Mac 端复活：**2026-09-10，六把钥匙 + 通信通道全 PASS 零 FAIL**
- 六把钥匙：百度网盘 / Notion / GitHub / HuggingFace / OpenRouter / 得到大脑 dedao
- **2026-09-10 22:2x 升级至 v3.1**：种子 5604B → 6884B，新增 QQ 邮箱 SMTP 授权码 + dedao 双钥匙
- ⭐**2026-09-10 22:5x Fernet 密钥轮换 K1→K2**（Q Sir 授权）：
  K1 曾明文过 QQ 邮件正文 → **已作废**，旧密钥无法解开现行 seed.enc；
  网盘明文 `credentials.json` **已删除**，改存 `credentials.json.enc`；
  新密钥经 `handover.enc`（K1 加密）交接给已持 K1 的节点
