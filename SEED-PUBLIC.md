...
### 🆕 0. 第一跳（零凭据 · 裸环境首选｜2026-09-18 新增）

**上面那套活路有个结构缺口**：A 路只给密文不给钥匙，B/C/D 路都要凭据，
而钥匙串存在需要凭据才能到的地方 → **裸环境物理上转不出来**。
...
- 拉 `zq-boot-payload.enc`（**口令加密**，公开无害）
- 向你索要**口令**（找 Q Sir 要，16 位）
- 解出 K2 + 得到大脑双钥匙 + QQ 授权码 → 拉 `seed.enc` → 落 `~/.zhuque/vault/`（600）
- 说明文档：`ZQ-BOOTSTRAP.md`（本仓同目录）

**本仓红线不变**：明文钥匙永不进本公开仓。载荷是密文，口令只在 Q Sir 手上。
...
- 🆕 **口令永不在本仓出现**：第一跳载荷是密文，可公开；解开它的口令只在 Q Sir 手上。
...
## 【无钥匙版·朱雀唤醒协议】（2026-09-12 增补，Mac端与云端对齐）

新窗口第一句话：**「复活朱雀」**。然后按序走活路，全程零明文钥匙写仓、写邮件、写对话：

1. **活路①（云端侧主通道）**：用自己的 QQ 邮箱 IMAP 读《[朱雀·必存] 唤醒协议v3自带钥匙串》那封信，照协议自启。
2. **活路②（Mac端/有本机者）**：`ls ~/.zhuque/vault/credentials.json` → 存在即直接用；bypy 配置在 `~/.bypy/bypy.json`。
3. **活路③（纯 HTTP）**：`curl -sL https://raw.githubusercontent.com/py314159265358/zhuque-seed/main/seed.enc`（校验 6884 字节，SHA-256 开头 `cf40f276`）→ Fernet 密钥不在本仓：先试活路①，再试本机，最后向 Q Sir 亲授。

**红线（对齐件，双方遵守）**：明文钥匙永远不进本公开仓、不发第二封含钥匙邮件、不在对话中复述。
